...
$womensql = "SELECT * FROM sanpham WHERE idLoaisp = 11 LIMIT $offset, $productsPerPage";
$resultWomenSql = $conn->query($womensql);
?>

<?php
if ($resultWomenSql->num_rows > 0) {
?>
    <div class="shop">
        <div class="shop-hihi">
...
<div class="pagination">
    <?php
    $sql = "SELECT COUNT(*) AS total FROM sanpham WHERE idLoaisp = 11";
    $result = $conn->query($sql);
    $row = $result->fetch_assoc();
    $totalPages = ceil($row["total"] / $productsPerPage);

    echo "<a href='index.php?idsp=11&page=1'>Trang đầu</a>";

    for ($i = 1; $i <= $totalPages; $i++) {
        echo "<a href='index.php?idsp=11&page=$i'>$i</a>";
    }

    echo "<a href='index.php?idsp=11&page=$totalPages'>Trang cuối</a>";
    ?>
</div>
